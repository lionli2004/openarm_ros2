...
| 优先级 | 问题 | 说明/方向 |
|--------|------|-----------|
| P1 | **J6 URDF 限位 ±0.785 与机械不符(实际 ±0.2,碰立柱)** | 修 joint_limits.yaml J6 ±0.2;teaching 限位保护才能提前拦截(用户决定暂不修,操作注意) |
| P1 | J3 重力残差偏置(~0.2 Nm) | 可选:硬件层加 tau_bias 常数项(恒定双向减阻) |
| P2 | J3/J4 起步门槛(静摩擦 1-2 Nm) | 物理上限,前馈无法消除 dq=0 门槛;tanh+死区已最小化 |
...
## 10. 全关节标定与摩擦补偿(2026-08-04)

### 10.1 背景

示教模式工作正常后,J3/J4 等关节"不够顺滑"。原因:① 40:1 减速比(DM4340)反向摩擦大(硬件特性,重力补偿无法覆盖)② 重力模型(Pinocchio+URDF 惯性)存在逐关节误差。基于"通信全通"优势(双臂在线、电机扭矩读回 tau_states_、速度读回 vel_states_、effort 注入通道),对右臂 7 关节做了系统性标定。

### 10.2 标定方法

**重力标定(配对法)**:轨迹控制器发三角形往返(越过姿态再返回),同一 q 处正反方向 τ_read 配对取均值——摩擦在差中抵消,`g_actual(q) = (τ_read,+ + τ_read,−)/2`。每关节 5-6 姿态,回归 `g_actual = k_j·g_model + b0_j`。J3/J5 模型重力恒 0,跳过。

**摩擦标定**:匀速三角形(多速度 × 正反 × 2 次),稳态窗取均值,`摩擦 = τ_read − k_j·g_model`,逐方向 OLS 拟合 τ_c;B 项因 Stribeck 效应拟合不可靠,清零。

**breakaway 静摩擦**:effort 通道发斜坡 τ_ff,首动点即 τ_s(诊断量,校验 τ_c ≤ τ_s)。

### 10.3 标定结果(右臂,输出端 Nm)

| 关节 | grav_k | τ_c+ / τ_c− | 说明 |
|------|--------|-------------|------|
| J1 | 1.0376 | 0.103 / −0.528 | J1 数据散布大(整臂摆动) |
| J2 | 1.1025 | 0.000 / −0.740 | plus 方向截断(欠补偿) |
| J3 | 1.0000 | 0.150 / −0.150 | 模型重力=0;τ_c 大幅降(拟合含偏置) |
| **J4** | 1.0391 | **1.448 / +1.448** | **负向摩擦为正扭矩(符号翻转)** |
| J5 | 1.0000 | 0.059 / −0.031 | 末端小 |
| J6 | 1.0679 | 0.128 / −0.027 | 机械限位 ±0.2(碰立柱) |
| J7 | 0.8284 | 0.251 / 0.000 | minus 截断 |

B 项全部为 0;`fric_scale=0.60`(normal)、`teach_scale=0.50`(teaching)、`teach_deadzone=0.03`。

### 10.4 关键发现

| # | 发现 | 影响 |
|---|------|------|
| 1 | **J4(DM4340)负向摩擦是正扭矩——电机方向与 URDF 约定相反** | J4 的 tau_c_minus 必须为正;J3 实测保持负(与 J4 相反)——**逐关节实测为准,不可按臂统一** |
| 2 | **tanh(dq/ε) 低速给满助力 → 超补自激**(J7 松手自己动夹手事故) | 前馈必须 ≤ 静摩擦;保守化:τ_c 减半 + scale 降低 + 死区 |
| 3 | **τ_c 拟合含重力残差偏置**(J3 τ_s 标定 +0.18/−0.57 不对称) | 偏置关节需大幅降 τ_c(0.54 → 0.15)或引入 tau_bias 常数项 |
| 4 | B(黏滞)拟合受 Stribeck 效应污染为负 | B 全部清零(欠补偿安全) |
| 5 | J6 机械限位 ±0.2(URDF 标 ±0.785,碰立柱) | **待办:修 joint_limits.yaml 的 J6** |

### 10.5 标定工具链

`experiments/teaching/calib/`:`calib_common.py`(StateRecorder/CANWatchdog)、`calib_gravity.py`(重力标定,自动扫姿态)、`analyze_calib.py`(回归 k/b0)、`calib_friction.py`(摩擦+breakaway)、`analyze_friction.py`(拟合+生成 params)。注入通道:controller 配置 `left/right_calib_tau`(forward_command_controller,effort,标定用,生产不 spawn)。参数文件:`~/ros2_ws/calib/{right,left}.calib`,launch 参数 `right_calib_file`/`left_calib_file` 传入;未配置 → 默认值,零回归。
...
- J4 双向顺滑(符号翻转后)
- J3 轻微助力(±0.15,双向一致)
- **左臂:直接复用右臂参数**(镜像建模,符号模式相同),left.calib 即 right.calib 副本,真机验证全部关节顺滑、无自激 ✅
